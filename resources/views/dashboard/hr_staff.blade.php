...
@section('content')
    ...
    <style>
        ...
        .db-stats-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 14px; margin-bottom: 18px; }
    ...
    <div class="db-topbar">
        ...
        <a href="{{ route('users.create') }}" class="db-topbar-btn">+ Add Staff</a>
    </div>

    <!-- Stats Cards -->
    <div class="db-stats-grid">
        <div class="db-stat-card">
            <div style="font: 700 10.5px 'Nunito Sans'; color: #98897A; text-transform: uppercase; letter-spacing: 0.6px;">Active staff</div>
            <div style="font: 600 26px 'Baloo 2'; color: #16436E;">{{ $activeStaffCount }}</div>
            <div style="font: 700 11.5px 'Nunito Sans'; color: #8A7D6C;">{{ $newHiresThisMonth }} started this month</div>
        </div>
        <div class="db-stat-card">
        ...
        <div class="db-stat-card">
            <div style="font: 700 10.5px 'Nunito Sans'; color: #98897A; text-transform: uppercase; letter-spacing: 0.6px;">Roles on team</div>
            <div style="font: 600 26px 'Baloo 2'; color: #16436E;">{{ $staffByRole->count() }}</div>
            <div style="font: 700 11.5px 'Nunito Sans'; color: #8A7D6C;">across the clinic</div>
        </div>
    </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 18px;">
            ...
            <div style="background: #FFFFFF; border: 1px solid #EBE4DA; border-radius: 14px; padding: 16px 20px;">
                <div class="db-card-head" style="display: flex; align-items: baseline; gap: 10px; margin-bottom: 12px;">
                    <div class="db-card-title" style="font: 600 16px 'Baloo 2'; color: #16436E; flex: 1;">Staff by role</div>
                    <a href="{{ route('users.index') }}" style="background: none; border: none; font: 800 12px 'Nunito Sans'; color: #C8355F; cursor: pointer; padding: 0; text-decoration: none;">All staff →</a>
                </div>
                @if ($staffByRole->isEmpty())
                    <div style="font: 700 12.5px 'Nunito Sans'; color: #98897A;">No staff records yet.</div>
                @else
                    <div style="display: flex; flex-direction: column; gap: 9px;">
                        @foreach ($staffByRole as $row)
                            <div style="display: flex; align-items: center; gap: 12px;">
                                <div style="width: 150px; font: 700 12.5px 'Nunito Sans'; color: #2B3A4C;">{{ Str::title(str_replace('_', ' ', strtolower($row->role))) }}</div>
                                <div style="flex: 1; height: 10px; background: #F3EDE3; border-radius: 5px; overflow: hidden;">
                                    <div style="width: {{ max(4, round($row->c / $staffByRoleMax * 100)) }}%; height: 100%; background: {{ $avatarColor($row->role) }}; border-radius: 5px;"></div>
                                </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 18px;">
            ...
            <div style="background: #FFFFFF; border: 1px solid #EBE4DA; border-radius: 14px; overflow: hidden;">
                <div style="display: flex; align-items: center; gap: 8px; padding: 16px 20px 10px;">
                    <div class="db-card-title" style="font: 600 16px 'Baloo 2'; color: #16436E; flex: 1;">Recently added staff</div>
                    <a href="{{ route('users.index') }}" style="background: none; border: none; font: 800 12px 'Nunito Sans'; color: #C8355F; cursor: pointer; padding: 0; text-decoration: none;">Open →</a>
                </div>
                @forelse ($recentStaff as $staff)
                    <a href="{{ route('users.show', $staff) }}" style="display: flex; gap: 11px; align-items: center; padding: 11px 20px; border-top: 1px solid #F3EDE3; cursor: pointer; text-decoration: none;">
                        <div style="width: 36px; height: 36px; border-radius: 50%; background: {{ $avatarColor($staff->id) }}; color: white; display: flex; align-items: center; justify-content: center; font: 600 13px 'Baloo 2'; flex-shrink: 0;">{{ strtoupper(substr($staff->first_name ?? '?', 0, 1)) }}</div>
                        <div style="flex: 1; min-width: 0;">
                            <div style="font: 800 13px 'Nunito Sans'; color: #2B3A4C;">{{ $staff->full_name }}</div>
                            <div style="font: 600 11.5px 'Nunito Sans'; color: #98897A; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $staff->job_title ?? $staff->roleLabel() }}{{ $staff->start_date ? ' · started ' . $staff->start_date->format('d M Y') : '' }}</div>
                        </div>
                    </a>
                @empty
                    <div style="padding: 24px 20px; text-align: center; font: 700 12.5px 'Nunito Sans'; color: #98897A;">No staff records yet.</div>
                @endforelse
            </div>
        </div>
    </div>
