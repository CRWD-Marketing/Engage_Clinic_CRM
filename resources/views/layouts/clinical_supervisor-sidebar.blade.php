...
<head>
    ...
    <title>@yield('title', 'Clinical Supervisor · Engage Clinic')</title>
...
<body class="admin-body">
    ...
    <div id="sidebarMobile" class="sidebar-mobile">
        ...
        <div class="flex items-center gap-3 p-3 rounded-xl bg-[#F8F5F0] mb-6">
            ...
            <div class="user-info">
                <div class="name">{{ Auth::user()->full_name ?? Auth::user()->name ?? 'Clinical Supervisor' }}</div>
                <div class="role">{{ Auth::user()->role ?? 'Clinical Supervisor' }}</div>
            </div>
        </div>
        ...
        <div class="waitlist-card mt-6">
            <div class="label">Therapist Oversight</div>
            <div class="number">{{ $activeTherapistsCount ?? '—' }} active</div>
            <div class="sub">session notes pending review</div>
        </div>
                ...
                <div onclick="toggleDropdown()" class="dropdown-trigger">
                    ...
                    <div style="flex: 1; min-width: 0;">
                        <div style="color: #2B3A4C; font-weight: 700; font-size: 12px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                            {{ Auth::user()->full_name ?? Auth::user()->name ?? 'Clinical Supervisor' }}
                        </div>
                        <div style="color: #98897A; font-size: 10px;">
                            {{ Auth::user()->role ?? 'Clinical Supervisor' }}
                        </div>
                    </div>
