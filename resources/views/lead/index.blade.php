...
@section('content')
    <style>
        ...
        /* Same slim scrollbar as the sidebar - no OS-default arrow buttons */
        ...
        .btn-danger {
            flex: 1; background: #B3261E; color: #fff; border: none; border-radius: 8px;
            padding: 10px 20px; font: 800 13px 'Nunito Sans'; cursor: pointer;
        }
        .btn-danger:hover { background: #92201A; }
        .btn-danger:disabled { opacity: 0.6; cursor: default; }

        .modal-box.modal-box-wide { width: 900px; }
        .term-table { width: 100%; border-collapse: collapse; font: 600 12.5px 'Nunito Sans'; }
        .term-table th {
            text-align: left; font: 700 10.5px 'Nunito Sans'; color: #98897A; text-transform: uppercase;
            letter-spacing: 0.5px; padding: 8px 10px; border-bottom: 1px solid #F3EDE3; white-space: nowrap;
        }
        .term-table td { padding: 10px; border-bottom: 1px solid #F3EDE3; vertical-align: top; color: #2B3A4C; }
        .term-table tr:last-child td { border-bottom: none; }
        .term-lead-name { font: 800 13px 'Nunito Sans'; color: #16436E; }
        .term-lead-parent { font: 600 11.5px 'Nunito Sans'; color: #98897A; }
        .term-badge { display: inline-block; padding: 3px 8px; border-radius: 6px; font: 800 11px 'Nunito Sans'; white-space: nowrap; }
        .btn-restore {
            background: #FFFFFF; color: #16436E; border: 1px solid #E2DACE; border-radius: 8px;
            padding: 7px 14px; font: 800 12px 'Nunito Sans'; cursor: pointer; white-space: nowrap;
        }
        .btn-restore:hover { background: #F6F3EE; }
        ...
        .ap-btn-convert:disabled { background: #EFE8DD; color: #B0A493; cursor: default; box-shadow: none; }
        ...
        .ic-field-input[type="number"]::-webkit-outer-spin-button,
        .ic-field-input[type="number"]::-webkit-inner-spin-button { -webkit-appearance: none; margin: 0; }
        .ic-field-input[type="number"] { -moz-appearance: textfield; }
    ...
    @php
        ...
        // Feeds the "Terminated leads - history" modal - built here rather
        // than in the controller since it just reshapes $leads (already
        // loaded for the board) for JS, not a new query.
        $terminatedLeadsList = $leads->where('status', \App\Models\Lead::STATUS_TERMINATED)
            ->sortByDesc(fn ($lead) => $lead->terminated_at ?? $lead->updated_at)
            ->values()
            ->map(fn ($lead) => [
                'id' => $lead->id,
                ...
                'enquired_label' => optional($lead->created_at)->diffForHumans(),
                'terminated_label' => optional($lead->terminated_at)->format('j M Y'),
                'termination_reason' => $lead->termination_reason,
                'termination_note' => $lead->termination_note,
                'status_before_termination' => $lead->status_before_termination,
                'status_before_termination_label' => $statusTagLabels[$lead->status_before_termination] ?? null,
                'estimated_value' => $lead->estimated_value_numeric,
            ]);
        $terminatedTotalValue = $terminatedLeadsList->sum('estimated_value');
    @endphp
        ...
        <div class="leads-container">
            ...
            <div class="kanban-topbar">
                ...
                <div class="topbar-buttons">
                    ...
                    <button class="btn-filter" id="terminatedToggleBtn" onclick="openTerminatedHistoryModal()">Terminated · {{ $terminatedCount }}</button>
    ...
    <!-- Action Panel (opens when a card's container is clicked - assign, schedule
         follow-up, log notes, or terminate. -->
    ...
    <!-- Terminate Lead Modal -->
    <div id="terminateModal" class="modal-overlay">
        <div class="modal-box modal-box-narrow">
            <div class="modal-header">
                <div style="flex: 1;">
                    <div class="modal-header-title" style="color: #B3261E;">Terminate lead</div>
                    <div class="modal-header-sub" id="terminateModalSub"></div>
                </div>
                <button class="modal-close-btn" onclick="closeTerminateModal()">✕</button>
            </div>

            <form onsubmit="return submitTerminateLead(event)">
                <div>
                    <div class="field-label">Reason *</div>
                    <select id="terminateReason" name="termination_reason" class="field-select" required>
                        <option value="">Select reason…</option>
                        @foreach (\App\Models\Lead::TERMINATION_REASONS as $reason)
                ...
                <div style="margin-top: 12px;">
                    ...
                    <input id="terminateNote" name="termination_note" type="text" placeholder="e.g. Daman declined pre-auth, family deferring to 2027" class="field-input">
                </div>
                <div style="display: flex; gap: 10px; margin-top: 16px;">
                    <button type="submit" id="terminateSubmitBtn" class="btn-danger">Terminate lead</button>
                    <button type="button" onclick="closeTerminateModal()" class="btn-cancel" style="flex: none; width: 110px;">Cancel</button>
                </div>
            </form>
        </div>
    </div>

    <!-- Terminated Leads History Modal -->
    <div id="terminatedHistoryModal" class="modal-overlay">
        <div class="modal-box modal-box-wide">
            <div class="modal-header">
                <div style="flex: 1;">
                    <div class="modal-header-title">Terminated leads — history</div>
                    <div class="modal-header-sub" id="terminatedHistorySub"></div>
                </div>
                <button class="modal-close-btn" onclick="closeTerminatedHistoryModal()">✕</button>
            </div>
            ...
            <div style="overflow-x: auto;">
                <table class="term-table">
                    ...
                    <tbody id="terminatedHistoryBody"></tbody>
                </table>
                <div id="terminatedHistoryEmpty" class="kanban-empty" style="display: none;">No terminated leads</div>
            </div>
        </div>
    </div>
@endsection
...
<script>
    ...
    const restoreLeadUrl = '{{ route("leads.restore", ["lead" => "__LEAD_ID__"]) }}';

    const terminatedLeadsData = @json($terminatedLeadsList);
    const terminatedTotalValue = {{ $terminatedTotalValue }};
    ...
    // ===================================================================
    // Action panel - opened by clicking a card's container. Handles the
    // full lead workflow: assign, schedule a follow-up, log a note,
    // terminate, or work through the intake checklist.
    // ===================================================================
    let apLead = null;
    ...
    function icSubmitStep(event, step) {
        ...
        .then(data => {
            ...
            const previousStatus = apLead.status;
            Object.assign(apLead, data.lead);
            renderApInfoGrid();
            icRenderChecklist();
            icCloseModal(step);

            const autoEnrolled = data.lead.status && data.lead.status !== previousStatus;
            if (autoEnrolled) {
                moveLeadCard(apLead.id, previousStatus, apLead.status);
                updateCounts(previousStatus, apLead.status);
                updateApStatusTag();
            }
            refreshApActions();

            showNotification(autoEnrolled ? 'Saved — all steps complete, lead moved to Enrolled.' : 'Saved.', 'success');
        })
        .catch(() => {
            saveBtn.disabled = false;
            saveBtn.textContent = originalText;
    ...
    function icAddServiceRow(existing) {
        ...
        row.innerHTML = `
            ...
            <div class="ic-insurance-fields" data-role="insurance-fields" style="display:none; gap:12px; flex-wrap:wrap;">
                <div style="flex:1; min-width:160px;">
                    <div class="ic-field-label">Approved hours</div>
                    <input type="number" min="0" class="ic-field-input" data-role="approved_hours" placeholder="e.g. 96" oninput="icUpdateServiceRowStatus(this)">
                </div>
                <div style="flex:1; min-width:160px;">
                    <div class="ic-field-label">Approval reference</div>
                    <input type="text" class="ic-field-input" data-role="approval_reference" placeholder="e.g. PA-2026-77341">
                </div>
            </div>
        `;
        ...
        if (existing) {
            ...
            if (existing.approved_hours) {
                row.querySelector('[data-role="approved_hours"]').value = existing.approved_hours;
            }
            if (existing.approval_reference) {
                row.querySelector('[data-role="approval_reference"]').value = existing.approval_reference;
            }
            icToggleInsuranceFields(row, existing.payer);
            icUpdateServiceRowStatus(row.querySelector('[data-role="hours"]'));
        }
    }

    function icToggleInsuranceFields(row, payerValue) {
        const fields = row.querySelector('[data-role="insurance-fields"]');
        const isInsurance = payerValue === 'Insurance';
        fields.style.display = isInsurance ? 'flex' : 'none';
        if (! isInsurance) {
            row.querySelector('[data-role="approved_hours"]').value = '';
            row.querySelector('[data-role="approval_reference"]').value = '';
        }
    }
    ...
    function icPickServiceRowValue(btn, role) {
        ...
        btn.classList.add('is-selected');
        if (role === 'payer') {
            icToggleInsuranceFields(btn.closest('.ic-service-row'), btn.dataset.value);
        }
    ...
    function icUpdateServiceRowStatus(el) {
        ...
        const isInsurance = payer?.dataset.value === 'Insurance';
        const approvedHours = row.querySelector('[data-role="approved_hours"]').value;
        const status = row.querySelector('.ic-service-status');

        const complete = service && payer && hours && (! isInsurance || approvedHours);
        status.textContent = complete ? 'Complete' : (isInsurance && service && payer && hours ? 'Needs approved hours' : 'Needs service, hours and payer');
    ...
    function icCollectServiceRows() {
        return Array.from(document.querySelectorAll('#icServiceRows .ic-service-row')).map(row => ({
            ...
            approved_hours: row.querySelector('[data-role="approved_hours"]').value || null,
            approval_reference: row.querySelector('[data-role="approval_reference"]').value || null,
    ...
    function refreshApActions() {
        ...
        if (currentStatus === 'enrolled') {
            ...
            // Status can reach "enrolled" (all 7 steps done, or a manual
            // Success click from Assessment done) without every checklist
            // step actually holding valid data - don't let conversion
            // proceed until intake_steps_complete genuinely says 7 of 7.
            const stepsComplete = apLead.intake_steps_complete ?? 0;
            const intakeComplete = stepsComplete >= 7;
            convertBtn.disabled = !intakeComplete;
            convertBtn.textContent = intakeComplete ? 'Convert to client' : `Finish intake first (${stepsComplete}/7)`;
            convertBtn.title = intakeComplete ? '' : 'Complete the remaining intake checklist steps before converting this lead.';
    ...
    // Terminating needs a reason on file (shown later in the history panel),
    // so this opens a small form instead of firing the status change straight
    // off a plain confirm() like the rest of the quick actions do.
    function terminateApLead() {
        if (!apLead) return;

        document.getElementById('terminateModalSub').textContent =
            [apLead.child_name, apLead.parent_guardian_name].filter(Boolean).join(' · ');
        document.getElementById('terminateReason').value = '';
        document.getElementById('terminateNote').value = '';
    ...
    function submitTerminateLead(event) {
        event.preventDefault();
        if (!apLead) return false;
        ...
        .then(data => {
            ...
            closeTerminateModal();
            closeActionPanel();
            showNotification('Lead terminated. Refreshing…', 'success');
            // A full reload is simplest here: termination touches the pipeline
            // counts, the topbar totals AND the terminated-history list all at
            // once, and it's a rare enough action that a reload doesn't hurt.
    ...
    // ===== Terminated leads - history modal =====
    function openTerminatedHistoryModal() {
        const body = document.getElementById('terminatedHistoryBody');
        const empty = document.getElementById('terminatedHistoryEmpty');
        body.innerHTML = '';

        document.getElementById('terminatedHistorySub').textContent =
            terminatedLeadsData.length + ' lead' + (terminatedLeadsData.length === 1 ? '' : 's') + ' closed · '
            + 'AED ' + terminatedTotalValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + '/mo lost value · most recent first';

        empty.style.display = terminatedLeadsData.length ? 'none' : 'block';

        terminatedLeadsData.forEach(lead => {
            const row = document.createElement('tr');

            const lostAt = lead.status_before_termination_label
                ? `<span class="term-badge" style="background:${(statusTagColors[lead.status_before_termination] || {}).bg || '#EDEFF1'}; color:${(statusTagColors[lead.status_before_termination] || {}).color || '#5A6B7E'};">${lead.status_before_termination_label}</span>`
                : '—';

            row.innerHTML = `
                <td>
                    <div class="term-lead-name">${lead.child_name || 'N/A'}${lead.child_age ? ' · ' + lead.child_age : ''}</div>
                    <div class="term-lead-parent">${lead.parent_guardian_name || ''}</div>
                </td>
                <td>
                    <div>${lead.terminated_label || '—'}</div>
                    <div class="term-lead-parent">${lead.enquired_label ? 'enquired ' + lead.enquired_label : ''}</div>
                </td>
                <td><span class="term-badge" style="background:#F9E4E2; color:#B3261E;">${lead.termination_reason || 'No reason given'}</span></td>
                <td>${lead.termination_note || ''}</td>
                <td>
                    ${lostAt}
                    <div class="term-lead-parent" style="margin-top:4px;">AED ${Number(lead.estimated_value || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}/mo</div>
                </td>
                <td><button type="button" class="btn-restore" onclick="restoreTerminatedLead(${lead.id}, this)">Restore</button></td>
            `;
            body.appendChild(row);
        });

        document.getElementById('terminatedHistoryModal').style.display = 'flex';
        document.body.style.overflow = 'hidden';
    }

    function closeTerminatedHistoryModal() {
        document.getElementById('terminatedHistoryModal').style.display = 'none';
        document.body.style.overflow = '';
    }

    function restoreTerminatedLead(leadId, btn) {
        btn.disabled = true;
        btn.textContent = 'Restoring…';

        fetch(restoreLeadUrl.replace('__LEAD_ID__', leadId), {
            method: 'POST',
            headers: { 'X-CSRF-TOKEN': csrfToken(), 'Accept': 'application/json' }
        })
        .then(response => response.json())
        .then(data => {
            if (!data.success) {
                showNotification(data.message || 'Could not restore this lead.', 'error');
                btn.disabled = false;
                btn.textContent = 'Restore';
                return;
            }

            showNotification('Lead restored. Refreshing…', 'success');
            setTimeout(() => location.reload(), 900);
        })
        .catch(() => {
            showNotification('Network error. Please try again.', 'error');
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // opens that lead's detail panel on load, mirroring the Contacts page's ?contact= param.
        const initialLeadParams = new URLSearchParams(window.location.search);
        const initialLeadId = initialLeadParams.get('lead');
        if (initialLeadId) {
            openActionPanel(initialLeadId);
            initialLeadParams.delete('lead');
            const remaining = initialLeadParams.toString();
        ...
        const terminatedHistoryModal = document.getElementById('terminatedHistoryModal');
        if (terminatedHistoryModal) {
            terminatedHistoryModal.addEventListener('click', function(e) {
                if (e.target === this) closeTerminatedHistoryModal();
            });
        }
